...
namespace P2DEngine
{
    public class myPhysicsBlock : myPhysicsGameObject
    {
        ...
        {
            private new Image image; // Store texture

            public WallBlock(float x, float y, float sizeX, float sizeY, Image texture)
                : base(x, y, sizeX, sizeY, Color.Black)
            {
                this.image = texture; // Store the texture for drawing
            }
